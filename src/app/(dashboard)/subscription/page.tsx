...
import { SubscriptionNotice } from "@/components/subscriptions/subscription-notice";
...
export default function SubscriptionPage() {
  ...
  const {
    ...
    isError,
  } = useSubscriptionUsage(businessId);
      ...
      <div className="flex min-h-[60vh] items-center justify-center text-center">
        <div className="max-w-md">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            We couldn't load your subscription information. Try refreshing the
            page.
          </p>
        </div>
      </div>
  ...
  if (!subscription) {
    return <NoSubscription businessName={selectedBusiness?.name} />;
  }
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
          View your current plan, account limits and workspace usage.
        </p>
      </div>
      ...
      <section className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            See how much of your current plan capacity you're using.
          </p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <UsageMeter
            ...
            icon={Building2}
            used={data?.usage.stores ?? 0}
            limit={subscription.limits.stores}
          />

          <UsageMeter
            title="Assigned review cards"
            description="Physical cards assigned to your workspace"
            icon={CreditCard}
            used={data?.usage.cards ?? 0}
      ...
      <section className="space-y-5 pt-2">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          ...
          <p className="text-xs text-muted-foreground">
            Plan changes are currently handled by ValYou.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              Contact ValYou to upgrade or change your plan. Online payment
              management will be available in a future version.
            </p>
          </div>
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center">
      <div className="max-w-md text-center">
        ...
        <h2 className="mt-5 text-xl font-semibold">No workspace assigned</h2>

        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          Subscription information becomes available after a ValYou business
          workspace is assigned to your account.
        </p>
...
function NoSubscription({ businessName }: { businessName?: string }) {
  return (
    <div className="flex min-h-[60vh] items-center justify-center">
      <div className="max-w-md text-center">
        ...
        <h2 className="mt-2 text-xl font-semibold">No active plan</h2>
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          Contact ValYou to activate a trial or subscription for this workspace.
        </p>
      </div>
    </div>
  );
